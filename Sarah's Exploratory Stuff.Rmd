...
```{r}
aa <- meat %>% 
  mean(reason_religious_practice_rank, na.rm = TRUE)
...
cc <- meat %>% 
  mean(reason_health_safety_rank, na.rm = TRUE)	
...
dd <- meat %>% 
  mean(reason_improve_taste_rank, na.rm = TRUE)	
...
ee <- meat %>% 
  mean(reason_improve_texture_rank, na.rm = TRUE)	
...
ff <- meat %>% 
  mean(reason_appearance_rank, na.rm = TRUE)	
...
f <- meat %>%
  ...
  geom_text(data=data.frame(x=ff ,y=0), aes(x + .4, y), label=ff, vjust= -2)

gg <- meat %>% 
  mean(reason_other_rank, na.rm = TRUE)
...
g <- meat %>%
  ...
  geom_text(data=data.frame(x=gg ,y=0), aes(x + .4, y), label=gg, vjust= -2)
...
```{r}
meat %>% 
  summarize(mean(reason_health_safety_rank, na.rm = TRUE),
            mean(reason_religious_practice_rank, na.rm = TRUE),
            mean(reason_nonreligious_custom_rank, na.rm = TRUE),
            mean(reason_improve_taste_rank, na.rm = TRUE),
            mean(reason_improve_texture_rank, na.rm = TRUE),
            mean(reason_appearance_rank, na.rm = TRUE),
            mean(reason_other_rank, na.rm = TRUE)) %>% 
